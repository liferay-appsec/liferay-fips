...
3. `jndi-keystore.bcfks` – Stores the encrypted database password used by the Tomcat `KeyStoreCredentialHandler`. The alias configured in `docker-compose.yaml` (`JNDI_DB_PASSWORD_ALIAS`) must exist inside this keystore, and its password must match `JNDI_KEYSTORE_PASSWORD`.
